...
async function getDocuSignClient() {
  ...
  console.log('[docusign-auth] config:', {
    oauthBase: OAUTH_BASE,
    integrationKey: integrationKey ? integrationKey.substring(0, 8) + '...' : 'MISSING',
    userId: userId || 'MISSING',
    baseUri: baseUri || 'MISSING',
    rsaKeyPresent: !!rsaKey,
    rsaKeyStartsCorrectly: rsaKey ? rsaKey.startsWith('-----BEGIN') : false,
    rsaKeyLength: rsaKey ? rsaKey.length : 0,
  });
  ...
  // Railway stores multiline as single line with literal \n — convert back
  rsaKey = rsaKey.replace(/\\n/g, '\n');

  const apiClient = new docusign.ApiClient();
  apiClient.setOAuthBasePath(OAUTH_BASE);

  // Scopes MUST be an array and MUST include 'impersonation' for JWT
