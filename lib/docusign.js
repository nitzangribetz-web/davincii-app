...
async function createDocuSignEnvelope({ artist, legalName, taxData }) {
  ...
  // Use absolute-position text tabs to place pre-filled data on the W-9.
  // DocuSign coordinate system: origin is top-left, units are 1/72 inch (points).
  // IRS W-9 Rev. March 2024, letter size 612 x 792 pts.
  // Calibrated from screenshot evidence: SSN at y=440 landed in EIN row (~30pts low).
  const textTabs = [];
  const checkboxTabs = [];

  // Line 1: Name — the fillable line below "1 Name of entity/individual"
  if (td.name) {
    textTabs.push({
      xPosition: '48',
      yPosition: '157',
  ...
  // Line 2: Business name — below "2 Business name/disregarded entity name"
  if (td.businessName) {
    textTabs.push({
      xPosition: '48',
      yPosition: '182',
  ...
  const classPositions = {
    individual: { x: '34', y: '224' },
    cCorp:      { x: '150', y: '224' },
    sCorp:      { x: '210', y: '224' },
    partnership: { x: '280', y: '224' },
    trustEstate: { x: '350', y: '224' },
    llc:         { x: '34', y: '248' },
    other:       { x: '120', y: '268' },
  };
  ...
  // Line 5: Address — fillable line below "5 Address"
  if (td.addressStreet) {
    textTabs.push({
      xPosition: '48',
      yPosition: '310',
  ...
  // Part I — SSN boxes (shifted up ~30pts from previous attempt)
  if (td.ssn) {
