...
async function getDocuSignClient() {
  ...
  let rsaKey = process.env.DOCUSIGN_RSA_KEY;
  if (!rsaKey) throw new Error('DOCUSIGN_RSA_KEY missing');
...
async function createDocuSignEnvelope({ artist, legalName, taxData }) {
  if (!process.env.DOCUSIGN_INTEGRATION_KEY) throw new Error('DOCUSIGN_INTEGRATION_KEY missing');
  if (!process.env.DOCUSIGN_ACCOUNT_ID) throw new Error('DOCUSIGN_ACCOUNT_ID missing');
