...
async function createDocuSignEnvelope({ artist, legalName, taxData }) {
  ...
  // Coordinates are in points (1/72 inch) from bottom-left of page.
  // IRS W-9 Rev. March 2024, letter size 612 x 792 pts.
  // DocuSign uses top-left origin, so y values are from the top.
  const textTabs = [];
  const checkboxTabs = [];

  // Line 1: Name (top of form, below header)
  if (td.name) {
    textTabs.push({
      xPosition: '42',
      yPosition: '185',
  ...
  // Positions for each checkbox on the W-9 (Rev. March 2024)
  const classPositions = {
    individual: { x: '42', y: '260' },
    cCorp:      { x: '172', y: '260' },
    sCorp:      { x: '240', y: '260' },
    partnership: { x: '310', y: '260' },
    trustEstate: { x: '375', y: '260' },
    llc:         { x: '42', y: '282' },
    other:       { x: '130', y: '300' },
  };
  ...
  // Part I — SSN boxes (right side of Part I section)
  if (td.ssn) {
    const ssn = td.ssn.replace(/\D/g, '');
    // Three separate SSN segments in the boxes
    textTabs.push({
      xPosition: '465',
      yPosition: '440',
      pageNumber: '1',
