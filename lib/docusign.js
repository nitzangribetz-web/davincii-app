 ...
 * Returns the envelope status string (e.g. 'completed', 'sent', 'delivered').
 */
async function getEnvelopeStatus(envelopeId) {
  const { baseUri } = getDocuSignConfig();
  const client = await getDocuSignClient();
  const accountId = baseUri.replace(/.*\/v2\.1\/accounts\//, '').replace(/\/.*/, '');
  const envelopesApi = new docusign.EnvelopesApi(client);
  const envelope = await envelopesApi.getEnvelope(accountId, envelopeId);
  return envelope.status; // 'completed', 'sent', 'delivered', 'voided', etc.
}

module.exports = { createDocuSignEnvelope, getDocuSignClient, diagnoseJwtAuth, getEnvelopeStatus };
